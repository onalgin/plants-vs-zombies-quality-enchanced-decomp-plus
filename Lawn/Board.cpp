...
Board::Board(LawnApp* theApp)
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mApp->IsLastStandLevel())
	{
		mStoreButton = new GameButton(1);
...
void Board::PickZombieWaves()
{
	...
	else
	{
		...
		if (mApp->IsSurvivalMode() || aGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mApp->IsLastStandLevel())
	...
	for (int aWave = 0; aWave < mNumWaves; aWave++)
	{
		...
		if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mApp->IsLastStandLevel())
		{
			aZombiePoints = (mChallenge->mSurvivalStage * GetNumWavesPerSurvivalStage() + aWave + 2) * 2 / 5 + 1;
		}
...
void Board::PickBackground()
{
	...
	case GameMode::GAMEMODE_LAST_STAND_DAY:
		mBackground = BackgroundType::BACKGROUND_1_DAY;
		break;
	...
	case GameMode::GAMEMODE_LAST_STAND_NIGHT:
		mBackground = BackgroundType::BACKGROUND_2_NIGHT;
		break;
	...
	case GameMode::GAMEMODE_LAST_STAND_POOL:
	case GameMode::GAMEMODE_LAST_STAND_ENDLESS:
		mBackground = BackgroundType::BACKGROUND_3_POOL;
		break;
	...
	case GameMode::GAMEMODE_LAST_STAND_FOG:
		mBackground = BackgroundType::BACKGROUND_4_FOG;
		break;
	...
	case GameMode::GAMEMODE_LAST_STAND_ROOF:
		mBackground = BackgroundType::BACKGROUND_5_ROOF;
		break;
...
void Board::InitLevel()
{
	...
	else if (aGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || aGameMode == GameMode::GAMEMODE_LAST_STAND_ROOF || aGameMode == GameMode::GAMEMODE_LAST_STAND_ENDLESS)
	{
		mSunMoney = 5000;
	}
	else if (aGameMode == GameMode::GAMEMODE_LAST_STAND_POOL || aGameMode == GameMode::GAMEMODE_LAST_STAND_FOG)
	{
		mSunMoney = 4000;
	}
	else if (aGameMode == GameMode::GAMEMODE_LAST_STAND_DAY || aGameMode == GameMode::GAMEMODE_LAST_STAND_NIGHT)
	{
		mSunMoney = 3000;
	}
	...
	else if (aGameMode == GameMode::GAMEMODE_LAST_STAND_DAY)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 8);
	...
	else if (aGameMode == GameMode::GAMEMODE_LAST_STAND_NIGHT)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 8);
	...
	else if (aGameMode == GameMode::GAMEMODE_LAST_STAND_POOL)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 8);
	...
	else if (aGameMode == GameMode::GAMEMODE_LAST_STAND_FOG)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 8);
		mSeedBank->mSeedPackets[0].SetPacketType(SeedType::SEED_LILYPAD);
	...
	else if (aGameMode == GameMode::GAMEMODE_LAST_STAND_ROOF)
	{
		TOD_ASSERT(mSeedBank->mNumPackets == 8);
...
void Board::InitLawnMowers()
{
	...
	if (aGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED || aGameMode == GameMode::GAMEMODE_CHALLENGE_BEGHOULED_TWIST ||
		...
		mApp->IsSquirrelLevel() || mApp->IsLastStandLevel() || mApp->IsIZombieLevel() || (StageHasRoof() && !mApp->mPlayerInfo->mPurchases[StoreItem::STORE_ITEM_ROOF_CLEANER]))
		return;
...
bool Board::ChooseSeedsOnCurrentLevel()
{
	...
	if (mApp->IsIZombieLevel() || mApp->IsSlotMachineLevel() || (mApp->IsLastStandLevel() && mApp->mGameMode != GameMode::GAMEMODE_LAST_STAND_ENDLESS))
		return false;
...
void Board::UpdateLevelEndSequence()
{
	if (mNextSurvivalStageCounter > 0)
	{
		...
		if (!mNextSurvivalStageCounter)
		{
			...
			else if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mApp->IsLastStandLevel())
			{
...
void Board::MouseDown(int x, int y, int theClickCount)
{
	...
	else if (mStoreButton && mStoreButton->IsMouseOver() && CanInteractWithBoardButtons() && theClickCount > 0)
	{
		...
		else if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mApp->mGameMode == GameMode::GAMEMODE_UPSELL || mApp->IsLastStandLevel())
		{
			mApp->PlaySample(Sexy::SOUND_GRAVEBUTTON);
		}
	}
...
int Board::GetLastStandFlagsCompleted()
{
	int aWavesPerFlag = GetNumWavesPerFlag();
	int aFlagsCompleted = mChallenge->mSurvivalStage * GetNumWavesPerSurvivalStage() / aWavesPerFlag;
	int aCurrentWave = mCurrentWave;
...
void Board::LastStandSaveScore()
{
	if (!mApp->IsLastStandLevel())
		return;

	int aFlagsCompleted = GetLastStandFlagsCompleted();
	int& aFlagsRecord = mApp->mPlayerInfo->mChallengeRecords[mApp->GetCurrentChallengeIndex()];
	if (aFlagsCompleted > aFlagsRecord)
	{
		aFlagsRecord = aFlagsCompleted;
...
{
	return ((mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND) || (mApp->IsLastStandLevel() && mApp->mGameMode != GameMode::GAMEMODE_LAST_STAND_ENDLESS)) && mChallenge->mSurvivalStage == LAST_STAND_FLAGS - 1;
}
...
{
	return ((mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND) || mApp->IsLastStandLevel()) && !IsLastStandFinalStage();
}
...
void Board::NextWaveComing()
{
	...
	{
		if (!IsSurvivalStageWithRepick() && mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_LAST_STAND && !mApp->IsContinuousChallenge() && !mApp->IsLastStandLevel())
		{
...
void Board::UpdateZombieSpawning()
{
	...
	if (mZombieCountDown == 0)
	{
		...
		else if (IsFlagWave(mCurrentWave) && (mApp->IsWallnutBowlingLevel() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mApp->IsLastStandLevel()))
		{
			mZombieHealthToNextWave = 0;
		...
		else
		{
			...
			if (mApp->IsLittleTroubleLevel() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_COLUMN || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mApp->IsLastStandLevel())
			{
...
void Board::Update()
{
	...
	mEffectCounter++;
	if (StageHasPool() && !mIceTrapCounter && mApp->mGameScene != GameScenes::SCENE_ZOMBIES_WON && (!mCutScene->IsSurvivalRepick() || !mCutScene->IsLastStandRepick()))
	{
...
void Board::DrawLevel(Graphics* g)
{
	...
	else
	{
		...
		if (mApp->IsSurvivalMode() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mApp->IsLastStandLevel())
		{
			int aFlags = GetSurvivalFlagsCompleted();
			if (aFlags > 0)
			{
...
void Board::DrawTopRightUI(Graphics* g)
{
	...
	if (mStoreButton && (mApp->mGameMode != GameMode::GAMEMODE_CHALLENGE_LAST_STAND || !mApp->IsLastStandLevel()))
	{
...
int Board::GetNumSeedsInBank()
{
	...
	if (mApp->mGameMode == GameMode::GAMEMODE_LAST_STAND_DAY || mApp->mGameMode == GameMode::GAMEMODE_LAST_STAND_NIGHT || mApp->mGameMode == GameMode::GAMEMODE_LAST_STAND_POOL || mApp->mGameMode == GameMode::GAMEMODE_LAST_STAND_FOG || mApp->mGameMode == GameMode::GAMEMODE_LAST_STAND_ROOF)
	{
		return 8;
	}
...
{
	if (mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mApp->IsSurvivalNormal(mApp->mGameMode) || mApp->IsLastStandLevel())
	{
		return 10;
	}
