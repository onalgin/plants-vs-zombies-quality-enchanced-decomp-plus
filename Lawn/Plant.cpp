...
void Plant::UpdateProductionPlant()
{
    ...
    if ((mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_LAST_STAND || mApp->IsLastStandLevel()) && mBoard->mChallenge->mChallengeState != ChallengeState::STATECHALLENGE_LAST_STAND_ONSLAUGHT)
        return;
...
void Plant::UpdateChomper()
{
    ...
    if (mState == PlantState::STATE_READY)
    {
        ...
        else if (FindTargetZombie(mRow, PlantWeapon::WEAPON_SECONDARY))
        {
            PlayBodyReanim("anim_bite", ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD, 20, 24.0f);
            mState = PlantState::STATE_CHOMPER_BITING_SECOND;
            mApp->PlayFoley(FoleyType::FOLEY_SQUASH_HMM);
            mStateCountdown = 70;
        }
    }
    ...
    else if (mState == PlantState::STATE_CHOMPER_BITING_SECOND)
    {
        if (mStateCountdown == 0)
        {
            mApp->PlayFoley(FoleyType::FOLEY_BIGCHOMP);

            Zombie* aZombie = FindTargetZombie(mRow, PlantWeapon::WEAPON_SECONDARY);
            bool doBite = false;
            if (aZombie)
            {
                if (aZombie->mZombieType == ZombieType::ZOMBIE_GARGANTUAR || aZombie->mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR ||
            ...
            {
                aZombie->DieWithLoot();
                mState = PlantState::STATE_CHOMPER_BITING_GOT_ONE_SECOND;
            }
        }
    }
    ...
    else if (mState == PlantState::STATE_CHOMPER_BITING_GOT_ONE_SECOND)
    {
        if (aBodyReanim->mLoopCount > 0)
        {
            PlayBodyReanim("anim_chew", ReanimLoopType::REANIM_LOOP, 0, 15.0f);
            ...
            mState = PlantState::STATE_CHOMPER_DIGESTING_SECOND;
            mStateCountdown = 2000;
        }
    }
    else if (mState == PlantState::STATE_CHOMPER_DIGESTING || mState == PlantState::STATE_CHOMPER_DIGESTING_SECOND)
    {
        if (mStateCountdown == 0)
        {
...
void Plant::UpdateReanim()
{
    ...
    if (mState == PlantState::STATE_CHOMPER_BITING_SECOND)
    {
        aOffsetX += 60.0f * aScaleX;
        aScaleX *= -1.0f;
    }

    if (mState == PlantState::STATE_CHOMPER_BITING_GOT_ONE_SECOND)
    {
        aOffsetX += 60.0f * aScaleX;
        aScaleX *= -1.0f;
    }

    if (mState == PlantState::STATE_CHOMPER_DIGESTING_SECOND)
    {
        aOffsetX += 60.0f * aScaleX;
        aScaleX *= -1.0f;
    }
...
void Plant::UpdateShooting()
{
    ...
    else if (mSeedType == SeedType::SEED_THREEPEATER)
    {
        int rowAbove = mRow - 1;
        int rowBelow = mRow + 1;
        bool ZombieBelow = FindTargetZombie(rowBelow, PlantWeapon::WEAPON_PRIMARY);
        bool ZombieRow = FindTargetZombie(mRow, PlantWeapon::WEAPON_PRIMARY);
        bool ZombieAbove = FindTargetZombie(rowAbove, PlantWeapon::WEAPON_PRIMARY);

        if (mShootingCounter == 35)
        {
            Reanimation* aHeadReanim2 = mApp->ReanimationGet(mHeadReanimID2);
            Reanimation* aHeadReanim3 = mApp->ReanimationGet(mHeadReanimID3);
            Reanimation* aHeadReanim1 = mApp->ReanimationGet(mHeadReanimID);

            if (ZombieBelow && ZombieRow && ZombieAbove)
            {
            ...
            else if (ZombieBelow && !ZombieRow && ZombieAbove)
            {
                int R = RandRangeInt(0, 1);
                if (R == 0)
                {
                    if (aHeadReanim1->mLoopType == ReanimLoopType::REANIM_PLAY_ONCE_AND_HOLD)
                    {
        ...
        if (mShootingCounter == 35 || mShootingCounter == 18)
        {
            Reanimation* aHeadReanim2 = mApp->ReanimationGet(mHeadReanimID2);
            Reanimation* aHeadReanim3 = mApp->ReanimationGet(mHeadReanimID3);
            Reanimation* aHeadReanim1 = mApp->ReanimationGet(mHeadReanimID);
    ...
    while (mBoard->IterateZombies(aZombie))
    {
        ...
        if (aZombie->EffectedByDamage(aDamageRangeFlags))
        {
            ...
            if (mSeedType == SeedType::SEED_CHOMPER)
            {
                ...
                if (aZombie == nullptr || mState == PlantState::STATE_CHOMPER_BITING_SECOND)
                {
                    aAttackRect.mX - 80;
                }
            }
...
Rect Plant::GetPlantAttackRect(PlantWeapon thePlantWeapon)
{
    ...
    {
        aRect = Rect(mX - 80, mY, mWidth, mHeight);
    }
    else if (thePlantWeapon == PlantWeapon::WEAPON_SECONDARY && mSeedType == SeedType::SEED_CHOMPER)
    {
