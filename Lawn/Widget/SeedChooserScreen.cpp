...
SeedChooserScreen::SeedChooserScreen()
{
	...
	mRepickButton = new GameButton(SeedChooserScreen::SeedChooserScreen_Repick);
	mRepickButton->mLabel = (_S("(RE-PICK)"));
	mRepickButton->mButtonImage = Sexy::IMAGE_BLANK;
	mRepickButton->mOverImage = Sexy::IMAGE_BLANK;
	mRepickButton->mDownImage = Sexy::IMAGE_BLANK;
	mRepickButton->SetFont(Sexy::FONT_BRIANNETOD12);
	mRepickButton->mColors[0] = Color(255, 255, 255);
	mRepickButton->mColors[1] = Color(200, 200, 200);
	mRepickButton->Resize(13, 85, 100, 30);
	mRepickButton->mParentWidget = this;
	...
	if (!mBoard->mCutScene->IsSurvivalRepick() || !mBoard->mCutScene->IsLastStandRepick())
	{
...
SeedChooserScreen::~SeedChooserScreen()
{
	...
	if (mRepickButton) delete mRepickButton;
}
...
{
	return (mApp->mGameMode == GAMEMODE_CHALLENGE_LAST_STAND || mApp->IsLastStandLevel()) && (theSeedType == SEED_SUNFLOWER || theSeedType == SEED_SUNSHROOM ||
...
void SeedChooserScreen::UpdateCursor()
{
	...
	if (mMouseVisible && mChooseState != CHOOSE_VIEW_LAWN && ((ZombieHitTest(mLastMouseX, mLastMouseY) && mApp->CanShowAlmanac() && !IsOverImitater(mLastMouseX, mLastMouseY)) || (aMouseSeedType != SEED_NONE && !SeedNotAllowedToPick(aMouseSeedType)) ||
		...
		mStoreButton->IsMouseOver() || mMenuButton->IsMouseOver() || mStartButton->IsMouseOver() || mRepickButton->IsMouseOver()))
...
void SeedChooserScreen::Update()
{
	...
	UpdateCursor();
	MarkDirty();
	mRepickButton->Update();
	if (mRepickButton)
	{
		bool hasSavedSeeds = false;
		if (mApp->mPlayerInfo)
		{
			for (int i = 0; i < mBoard->mSeedBank->mNumPackets; i++)
			{
				if (mApp->mPlayerInfo->save_seedTypes[i] != -1)
				{
					hasSavedSeeds = true;
					break;
				}
		...
		mRepickButton->SetDisabled(!hasSavedSeeds);
		...
		{
			mRepickButton->mColors[0] = Color(128, 128, 128);
			mRepickButton->mColors[1] = Color(128, 128, 128);
		}
	}
}
...
void SeedChooserScreen::OnStartButton()
{
	...
	if (!PickedPlantType(SEED_SUNFLOWER) && !PickedPlantType(SEED_TWINSUNFLOWER) && !PickedPlantType(SEED_SUNSHROOM) &&
		(!mBoard->mCutScene->IsSurvivalRepick() || !mBoard->mCutScene->IsLastStandRepick()) && (mApp->mGameMode != GAMEMODE_CHALLENGE_LAST_STAND || !mApp->IsLastStandLevel()))
	{
	...
	if (mBoard->StageHasPool() && !PickedPlantType(SEED_LILYPAD) && !PickedPlantType(SEED_SEASHROOM) && !PickedPlantType(SEED_TANGLEKELP) && (!mBoard->mCutScene->IsSurvivalRepick() || !mBoard->mCutScene->IsLastStandRepick()))
	{
...
void SeedChooserScreen::RepickSeeds()
{
	if (mApp->mPlayerInfo)
	{
		// Сбрасываем все семена обратно в выбор
		for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER; aSeedType = (SeedType)(aSeedType + 1))
		{
			ChosenSeed& aChosenSeed = mChosenSeeds[aSeedType];
		...
		// Загружаем сохраненные семена
		for (int anIndex = 0; anIndex < mBoard->mSeedBank->mNumPackets; anIndex++)
		{
			int seedIndex = mApp->mPlayerInfo->save_seedTypes[anIndex];

			if (seedIndex == -1)
				continue;

			if (seedIndex > (int)NUM_SEEDS_IN_CHOOSER) {
				// Имитатор - ручная установка без вызова ClickedSeedInChooser
				SeedType imitaterType = (SeedType)(seedIndex - (int)NUM_SEEDS_IN_CHOOSER);
				ChosenSeed& aChosenSeed = mChosenSeeds[SEED_IMITATER];

				// Устанавливаем тип имитации
				aChosenSeed.mImitaterType = imitaterType;

				// Ручная установка в банк
				aChosenSeed.mTimeStartMotion = 0;
				aChosenSeed.mTimeEndMotion = 0;
				aChosenSeed.mStartX = aChosenSeed.mX;
				aChosenSeed.mStartY = aChosenSeed.mY;
				...
				// Обычное растение
				SeedType targetSeedType = (SeedType)seedIndex;
				if (mApp->SeedTypeAvailable(targetSeedType) &&
					mChosenSeeds[targetSeedType].mSeedState == SEED_IN_CHOOSER)
				{
					ClickedSeedInChooser(mChosenSeeds[targetSeedType]);
				}
			}
		}
...
void SeedChooserScreen::MouseDown(int x, int y, int theClickCount)
{
	...
	else if (mRepickButton->IsMouseOver())
	{
		mApp->PlaySample(Sexy::SOUND_TAP);
		ButtonDepress(SeedChooserScreen::SeedChooserScreen_Repick);
	}
	else
	{
...
void SeedChooserScreen::CloseSeedChooser()
{
	if (mApp->mPlayerInfo)
	{
		for (int i = 0; i < mBoard->mSeedBank->mNumPackets; i++)
		{
			SeedType seed = FindSeedInBank(i);
			if (seed == SEED_IMITATER) {
				// Для имитатора сохраняем тип растения который он имитирует + смещение
				ChosenSeed& imitaterSeed = mChosenSeeds[SEED_IMITATER];
				if (imitaterSeed.mImitaterType != SEED_NONE) {
					mApp->mPlayerInfo->save_seedTypes[i] = (int)imitaterSeed.mImitaterType + (int)NUM_SEEDS_IN_CHOOSER;
				}
				else {
					mApp->mPlayerInfo->save_seedTypes[i] = -1; // Имитатор без типа
				}
			}
			else if (seed != SEED_NONE) {
				// Обычное растение
				mApp->mPlayerInfo->save_seedTypes[i] = (int)seed;
			}
			else {
				// Пустой слот
				mApp->mPlayerInfo->save_seedTypes[i] = -1;
			}
		}
	}
