...
bool Zombie::TrySpawnLevelAward()
{
    ...
    else if (mBoard->IsLastStandStageWithRepick())
    {
        ...
        mBoard->FadeOutLevel();
        mApp->PlayFoley(FoleyType::FOLEY_SPAWN_SUN);
        if (mApp->mGameMode == GameMode::GAMEMODE_LAST_STAND_ENDLESS || mApp->mGameMode == GameMode::GAMEMODE_LAST_STAND_ROOF)
        {
            for (int i = 0; i < 20; i++)
            {
        ...
        if (mApp->mGameMode == GameMode::GAMEMODE_LAST_STAND_FOG || mApp->mGameMode == GameMode::GAMEMODE_LAST_STAND_POOL)
        {
            for (int i = 0; i < 16; i++)
            {
                mBoard->AddCoin(aCenterX + i * 5, aCenterY, CoinType::COIN_SUN, CoinMotion::COIN_MOTION_COIN);
