...
void CutScene::PlaceLawnItems()
{
	...
	if (!IsSurvivalRepick() || !IsLastStandRepick())
	{
...
bool CutScene::IsLastStandRepick()
{
	return (mApp->IsLastStandLevel() && mBoard->mChallenge->mSurvivalStage > 0 && mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO);
}
...
{
	if (mBoard->mChallenge->mSurvivalStage > 0 && (mApp->IsSurvivalMode() || mApp->mGameMode == GameMode::GAMEMODE_LAST_STAND_ENDLESS))
		return false;  
...
void CutScene::StartLevelIntro()
{
	...
	if (mBoard->StageHasGraveStones())
	{
		...
		else if (!IsSurvivalRepick() || !IsLastStandRepick())
		{
