...
LawnApp::LawnApp()
{
	...
	mTitle = _S("Plants vs. Zombies: Quality Enhanced Plus " + mVersion);
...
bool LawnApp::UpdatePlayerProfileForFinishingLevel()
{
	...
	else if (IsPuzzleMode())
	{
		...
		if (!HasFinishedAdventure() && (mGameMode == GameMode::GAMEMODE_SCARY_POTTER_3 || mGameMode == GameMode::GAMEMODE_PUZZLE_I_ZOMBIE_3 || mGameMode == GameMode::GAMEMODE_LAST_STAND_POOL))
		{
		...
		if (aUnlockedNewChallenge)
		{
			...
			else if (IsLastStandLevel())
			{
				mPlayerInfo->mHasNewLastStand = 1;
			}
			else
			{
...
bool LawnApp::IsEndlessLastStand(GameMode theGameMode)
{
	return theGameMode == GameMode::GAMEMODE_LAST_STAND_ENDLESS;
}
...
{
	return mBoard && mGameMode >= GameMode::GAMEMODE_LAST_STAND_DAY && mGameMode <= GameMode::GAMEMODE_LAST_STAND_ENDLESS;
}
...
bool LawnApp::HasBeatenChallenge(GameMode theGameMode)
{
	...
	if (IsSurvivalEndless(theGameMode) || IsEndlessScaryPotter(theGameMode) || IsEndlessIZombie(theGameMode) || IsEndlessLastStand(theGameMode))
	{
		return false;
	}
